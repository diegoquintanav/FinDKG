...
with open(
    os.path.join(log_root_path, f"{args.graph}_args_{args.version}.txt"), "w"
) as f:
    f.write(pprint.pformat(args.__dict__))

add_logger_file_handler(args.graph, args.version, ".", log_time=None, fname_prefix="")
...
init_dynamic_relation_embeds = MultiAspectEmbedding(
    structural=get_embedding(
        G.num_relations, [args.num_rnn_layers, args.rel_embed_dim, 2], zero_init=True
    ),
    temporal=get_embedding(
        G.num_relations, [args.num_rnn_layers, args.rel_embed_dim, 2], zero_init=True
    ),
