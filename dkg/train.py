...
def main(args):
    ...
    time_interval_transform = TimeIntervalTransform(
        log_transform=args.time_interval_log_transform
    )
    ...
    if args.static_dynamic_combine_mode == "static_only":
        assert args.embedding_updater_structural_gconv is None, (
            args.embedding_updater_structural_gconv
        )
        assert args.embedding_updater_temporal_gconv is None, (
            args.embedding_updater_temporal_gconv
        )
    ...
    static_entity_embeds = MultiAspectEmbedding(
        structural=get_embedding(
            G.num_nodes(), args.static_entity_embed_dim, zero_init=False
        ),
    ...
    init_dynamic_entity_embeds = MultiAspectEmbedding(
        structural=get_embedding(
            G.num_nodes(),
            [args.num_rnn_layers, args.structural_dynamic_entity_embed_dim],
            zero_init=True,
        ),
        temporal=get_embedding(
            G.num_nodes(),
            [args.num_rnn_layers, args.temporal_dynamic_entity_embed_dim, 2],
            zero_init=True,
        ),
    )
    init_dynamic_relation_embeds = MultiAspectEmbedding(
        structural=get_embedding(
            G.num_relations,
            [args.num_rnn_layers, args.rel_embed_dim, 2],
            zero_init=True,
        ),
        temporal=get_embedding(
            G.num_relations,
    ...
    run_best_checkpoint_prefix = (
        f"{args.graph}_evokg_{args.exp_time}_run_best_checkpoint"
    )
    ...
    with open(
        os.path.join(log_root_path, f"{args.graph}_evokg_args_{args.exp_time}.txt"), "w"
    ) as f:
    ...
    time_optimizer = torch.optim.AdamW(
        params, lr=args.lr, weight_decay=args.weight_decay
    )

    val_static_entity_emb = (
        None  # static entity embeddings after processing up to validation data
    )
    val_dynamic_entity_emb = (
        None  # dynamic entity embeddings after processing up to validation data
    )
    val_dynamic_relation_emb = (
        None  # dynamic relation embeddings after processing up to validation data
    )
    node_latest_event_time_post_valid = (
        None  # node latest event time after processing up to validation data
    )
    loss_weights = None
    ...
    if args.load_best:
        if os.path.isfile(args.load_best):
            ...
            overall_best_checkpoint_fpath = stopper.overall_best_checkpoint_fpath(
                args.load_best
            )

        if not os.path.exists(overall_best_checkpoint_fpath):
            logger.info(
                f"'--load-best {args.load_best}' ignored. No best model saved at {overall_best_checkpoint_fpath}."
            )
        else:
            ...
            best_score = (
                ""
                if "score" not in best_checkpoint
                else f"best score={best_checkpoint['score']:.4f}"
            )
            desc = list(filter(None, [opt, best_score]))
            desc = f"({', '.join(desc)}) " if desc else ""
            logger.info(
                f"Loaded the best model {desc}from {overall_best_checkpoint_fpath}."
            )
    ...
    try:
        for epoch in range(args.epochs):
            ...
            for batch_i, (prior_G, batch_G, cumul_G, _batch_times) in enumerate(
                ...
                if batch_i > 0 and (
                    (batch_i % args.rnn_truncate_every == 0) or last_batch
                ):
                    ...
                    if (
                        args.embedding_updater_structural_gconv
                        or args.embedding_updater_temporal_gconv
                    ):
            ...
            if epoch >= args.eval_from and epoch % args.eval_every == 0:
                ...
                (
                    ...
                    args,
                    "Validation",
                    args.full_link_pred_validation,
                    args.time_pred_eval,
                    epoch,
                )
        ...
        if node_latest_event_time_post_valid is None:  # validation was not performed
            val_static_entity_emb = static_entity_embeds
            val_dynamic_entity_emb, val_dynamic_relation_emb = forward_graphs(
                model,
        ...
        if args.clean_up_run_best_checkpoint:
            ...
            logger.info(
                f"Removed the run best checkpoint ({stopper.run_best_checkpoint_fpath})"
            )
        ...
        for data_loader in data_loaders:
            ...
            for batch_i, (prior_G, batch_G, cumul_G, batch_times) in enumerate(
                ...
                data_loader_tqdm.set_description(
                    f"[Forwarding: batch_times={batch_times[0]}-{batch_times[-1]}]"
                )
...
def unpack_checkpoint(
    checkpoint: dict,
    model: DynamicGraphModel,
    edge_optimizer: Optional[torch.optim.AdamW] = None,
    time_optimizer: Optional[torch.optim.AdamW] = None,
) -> tuple[
    DynamicGraphModel,
    ...
    return (
        ...
        edge_optimizer,
        time_optimizer,
        checkpoint["static_entity_emb"],
        checkpoint["init_dynamic_entity_emb"],
        checkpoint["val_dynamic_entity_emb"],
        checkpoint["init_dynamic_relation_emb"],
        checkpoint["val_dynamic_relation_emb"],
        node_latest_event_time,
...
def compute_loss_static(model, batch_G, static_entity_emb, args, batch_eid=None):
    ...
    sub_batch_G_static_entity_emb = static_entity_emb[
        sub_batch_G.ndata[dgl.NID].long()
    ].to(args.device)

    # Compute the edge loss for the subgraph.
    edge_LL = model.edge_model(
        sub_batch_G, sub_batch_G_static_entity_emb, eid=batch_eid
    )
    loss_dict["edge"] = -edge_LL
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Name of graph/dataset",
    )
    parser.add_argument("--seed", type=int, default=101, help="random seed")
    parser.add_argument(
        "--gpu", type=int, default=0, help="which GPU to use. Set to -1 to use CPU."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="indicates whether to apply early stoping or not",
    )
    parser.add_argument(
        "--patience",
        type=int,
        default=30,
        help="number of times to be tolerate until early stopping",
    )
    parser.add_argument(
        "--early-stop-criterion",
        type=str,
        default="MRR",
        choices=["MRR", "MAE", "loss"],
    )
    parser.add_argument(
        "--eval-every", type=int, default=1, help="perform evaluation every k epoch(s)"
    )
    parser.add_argument(
        "--eval-from",
        type=int,
        default=0,
        help="perform evaluation starting from the given epoch",
    )
    parser.add_argument(
        "--full-link-pred-validation",
        dest="full_link_pred_validation",
        action="store_true",
        help="if given, perform full link prediction for validation",
    )
    parser.set_defaults(full_link_pred_validation=False)
    parser.add_argument(
        "--full-link-pred-test",
        dest="full_link_pred_test",
        action="store_true",
        help="if given, perform full link prediction for testing",
    )
    ...
    parser.add_argument(
        ...
        dest="time_pred_eval",
        action="store_true",
        help="if given, perform time prediction",
    )
    ...
    parser.add_argument(
        "--load-best",
        default=None,
        help="'edge', 'time', 'both', or path of the best model",
    )
    parser.add_argument(
        "--best-checkpoint-prefix",
        default=None,
        type=str,
        help="prefix of the checkpoint file with the overall best model",
    )
    parser.add_argument(
        "--run-best-checkpoint-prefix",
        default=None,
        type=str,
        help="prefix of the checkpoint file with the run best model",
    )
    parser.add_argument(
        "--clean-up-run-best-checkpoint",
        dest="clean_up_run_best_checkpoint",
        action="store_true",
    ...
    parser.add_argument(
        "--log-dir", type=str, default="evokg", help="log directory name"
    )
    parser.add_argument(
        "--embedding-updater-structural-gconv",
        type=nullable_string,
        choices=["RGCN+RNN", "RGCN+GRU", None],
        default="RGCN+RNN",
        help="graph conv module for embedding updater",
    )
    parser.add_argument(
        "--embedding-updater-temporal-gconv",
        type=nullable_string,
        choices=["RGCN+RNN", "RGCN+GRU", None],
        default="RGCN+RNN",
        help="graph conv module for embedding updater",
    ...
    parser.add_argument(
        "--embedding-updater-activation",
        type=activation_string,
        default="tanh",
        help="activation for embedding updater",
    )
    parser.add_argument(
        "--num-gconv-layers",
        type=int,
    ...
    parser.add_argument(
        "--num-rnn-layers", type=int, default=1, help="number of recurrent layers"
    )
    ...
    parser.add_argument(
        ...
        type=nullable_string,
        default=None,
        help="graph conv module for combiner",
    )
    parser.add_argument(
        "--combiner-activation",
        type=activation_string,
        default="tanh",
        help="activation for combiner",
    )
    parser.add_argument(
        "--static-dynamic-combine-mode", choices=["concat"], default="concat"
    )
    parser.add_argument(
        "--inter-event-time-mode",
        choices=["node2node_inter_event_times", "min_inter_event_times"],
        default="node2node_inter_event_times",
    )
    parser.add_argument(
        "--time-interval-no-log-transform",
        dest="time_interval_log_transform",
        action="store_false",
    )
    ...
    add_logger_file_handler(
        ...
        args.log_dir,
        args.exp_time,
        fname_prefix=args.result_file_prefix,
    )
