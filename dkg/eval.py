...
from dkg.model import DynamicGraphModel
...
def evaluate(
    ...
    entire_G,
    static_entity_emb,
    init_dynamic_entity_emb,
    init_dynamic_relation_emb,
    num_relations,
    args,
    phase,
    full_link_pred_eval,
    ...
    with torch.no_grad():
        if phase != "Train":
            ...
            log_msg += (
                f"{phase} loss total={sum([sum(l) for l in eval_loss_dict.values()]):.4f} | "
        ...
        if time_pred_eval:
            ...
            for i, (prior_G, batch_G, cumul_G, batch_times) in enumerate(batch_tqdm):
                ...
                if phase in ["Test", "Validation"]:
                    batch_eval_dict = eval_time_prediction(
                        model,
                        batch_G,
                        static_entity_emb,
                        dynamic_entity_emb,
                    ...
                    logger.info(
                        f"[batch-{i}] MAE={batch_eval_dict['MAE']:.4f}, RMSE={batch_eval_dict['RMSE']:.4f}"
                    )
            ...
            eval_dict["RMSE"] = RegressionMetric.root_mean_squared_error(
                eval_time_diffs
            )
            logger.info(
                log_msg + f", MAE={eval_dict['MAE']:.4f}, RMSE={eval_dict['RMSE']:.4f}"
            )
            ...
            if phase == "Test":
                ...
                with open(
                    os.path.join(
                        log_root_path,
                        f"{args.result_file_prefix}{args.graph}_eval_{args.eval}_time_pred_test_result.txt",
                    ),
                    "w",
                ) as f:
                    f.write(
                        f"{args.seed},{eval_dict['MAE']:.6f},{eval_dict['RMSE']:.6f}\n"
                    )
        ...
        if full_link_pred_eval:
            ...
            for i, (prior_G, batch_G, cumul_G, batch_times) in enumerate(batch_tqdm):
                ...
                if (
                    ...
                    or (
                        phase == "Validation"
                        and args.graph == settings.GRAPH_ICEWS18
                        and i % 6 == 0
                    )
                    or (
                        phase == "Validation"
                        and args.graph == settings.GRAPH_ICEWS14
                        and i % 4 == 0
                    )
                    or (
                        phase == "Validation"
                        and args.graph == settings.GRAPH_GDELT
                        and i % 20 == 0
                    )
                    or (
                        phase == "Validation"
                        and args.graph == settings.GRAPH_YAGO
                        and i % 2 == 0
                    )
                    or (
                        phase == "Validation"
                        and args.graph == settings.GRAPH_WIKI
                        and i % 3 == 0
                    )
                    or (phase == "Validation" and args.eval == "edge")
                ):
                    ...
                    batch_eval_dict = {
                        "MRR": RankingMetric.mean_reciprocal_rank(
                            batch_eval_edge_ranks
                        ),
            ...
            if loss_weights is None:
                ...
                mrr_list = [
                    RankingMetric.mean_reciprocal_rank(eval_ranks)
                    for eval_ranks in eval_ranks_dict.values()
                ]
                max_mrr_idx, min_mrr_idx = np.argmax(mrr_list), np.argmin(mrr_list)
                best_weights, worst_weights = (
                    weights_list[max_mrr_idx],
                    weights_list[min_mrr_idx],
                )
            ...
            logger.info(
                log_msg
                + f", MRR={eval_dict['MRR']:.6f}, Rec@1={eval_dict['REC1']:.6f}, Rec@3={eval_dict['REC3']:.6f}, "
            ...
            if phase == "Test":
                ...
                with open(
                    os.path.join(
                        log_root_path,
                        f"{args.result_file_prefix}{args.graph}_eval_{args.eval}_link_pred_test_result.txt",
                    ),
                    "w",
                ) as f:
                    f.write(
                        f"{args.seed},{eval_dict['REC1']:.6f},{eval_dict['REC3']:.6f},{eval_dict['REC10']:.6f},{eval_dict['MRR']:.6f}\n"
                    )
...
def eval_link_prediction(
    ...
    with torch.no_grad():
        ...
        eval_eid = (
            torch.nonzero(
                (eval_time_from <= cumul_G.edata["time"])
                & (cumul_G.edata["time"] <= eval_time_to),
                as_tuple=False,
            )
            .squeeze()
            .view(-1)
        ...
        assert (
            len(dynamic_entity_emb.structural)
            == len(dynamic_entity_emb.structural)
            == len(static_entity_emb.structural)
            == len(static_entity_emb.temporal)
        )

        with cumul_G.local_scope():
            """Compute edge likelihood"""
            edges_target_entity_log_prob = (
                torch.empty(len(eval_eid), entire_G.num_nodes()).fill_(-1e20).cpu()
            )  # shape: (# edges in cumul_G & belonging to eval_times, # nodes in the entire graph)
            if args.eval in ["edge", "both"]:
                cumul_G_structural_static_emb = static_entity_emb.structural[
                    cumul_G.ndata[dgl.NID].long()
                ].to(args.device)
                cumul_G_structural_dynamic_emb = dynamic_entity_emb.structural[
                    cumul_G.ndata[dgl.NID].long()
                ][:, -1, :].to(args.device)
                cumul_G_structural_combined_emb = model.combiner(
                    cumul_G_structural_static_emb,
                    cumul_G_structural_dynamic_emb,
                    cumul_G,
                )
                structural_dynamic_relation_emb = dynamic_relation_emb.structural[
                    :, -1, :, :
                ].to(args.device)  # [:, -1, :, :] to retrieve last hidden from rnn
                ...
                _, edges_head_pred, edges_rel_pred, edges_tail_pred = model.edge_model(
                    cumul_G,
                    cumul_G_structural_combined_emb,
                    cumul_G_structural_static_emb,
                    cumul_G_structural_dynamic_emb,
                    structural_dynamic_relation_emb,
                    eid=eval_eid,
                    return_pred=True,
                )
                edges_target_entity_pred = edges_tail_pred
                edges_target_entity_log_prob = (
                    torch.log(torch.softmax(edges_target_entity_pred, dim=1))
                    .cpu()
                    .detach()
                )
            ...
            edges_time_log_prob = (
                torch.empty(len(eval_eid), entire_G.num_nodes()).fill_(-1e20).cpu()
            )  # shape=(# edges in cumul_G & belonging to eval_times, # node in the entire graph)
            if args.eval in ["time", "both"]:
                cumul_G_temporal_static_emb = static_entity_emb.temporal[
                    cumul_G.ndata[dgl.NID].long()
                ].to(args.device)
                cumul_G_temporal_dynamic_emb = dynamic_entity_emb.temporal[
                    cumul_G.ndata[dgl.NID].long()
                ][:, -1, :, :].to(args.device)
                temporal_dynamic_relation_emb = dynamic_relation_emb.temporal[
                    :, -1, :, :
                ].to(args.device)  # [:, -1, :, :] to retrieve last hidden from rnn

                time_log_prob_eval_dict = eval_edges_iet(
                    model,
                    cumul_G,
                    cumul_G_temporal_dynamic_emb,
                    cumul_G_temporal_static_emb,
                ...
                edges_time_log_prob_cumul_G = time_log_prob_eval_dict[
                    "edge_scores"
                ]  # shape=(# edges in cumul_G & belonging to eval_times, # nodes in cumul_G)
                edges_time_log_prob[:, cumul_G.ndata[dgl.NID].cpu().detach().long()] = (
                    edges_time_log_prob_cumul_G  # ignore nodes that are not yet added to the graph
                )

            """Evaluate triples"""
            entire_G_edges_src, entire_G_edges_dst = entire_G.edges()
            entire_G_edges_rel = entire_G.edata["rel_type"]
            entire_G_edges = torch.cat(
                (
                    entire_G_edges_src.view(-1, 1),
                    entire_G_edges_rel.view(-1, 1),
                    entire_G_edges_dst.view(-1, 1),
                ),
                dim=1,
            ).to(args.device)

            cumul_G_edges_src, cumul_G_edges_dst = cumul_G.edges()
            eval_triples = torch.cat(
                (
                    cumul_G.ndata[dgl.NID][cumul_G_edges_src[eval_eid].long()].view(
                        -1, 1
                    ),
                    cumul_G.edata["rel_type"][eval_eid].view(-1, 1),
                    cumul_G.ndata[dgl.NID][cumul_G_edges_dst[eval_eid].long()].view(
                        -1, 1
                    ),
                ),
                dim=1,
            ...
            assert (
                len(eval_triples)
                == len(edges_target_entity_log_prob)
                == len(edges_time_log_prob)
            ), (eval_triples.shape, edges_target_entity_log_prob.shape)

            batch_G_inter_event_times = EventTimeHelper.get_sparse_inter_event_times(
                batch_G, model.node_latest_event_time[..., 0]
            )
            ...
            if args.eval == "edge":
                ...
                time_log_prob_weights = np.linspace(1.0, 0.0, num=11).tolist() + [
                    1.5,
                    2.0,
                    3.0,
                ]
                log_prob_weights_list = [
                    (1.0, time_weight) for time_weight in time_log_prob_weights
                ]
            ...
            eval_ranks_dict = OrderedDict(
                [(weights, []) for weights in log_prob_weights_list]
            )
            for (
                s,
                r,
                o,
            ), target_entity_log_prob, time_log_prob, inter_event_time in zip(
                eval_triples,
                edges_target_entity_log_prob,
                edges_time_log_prob,
                batch_G_inter_event_times,
            ):
                assert target_entity_log_prob.shape == time_log_prob.shape, (
                    target_entity_log_prob.shape,
                    time_log_prob.shape,
                )
                ...
                for log_prob_weights in log_prob_weights_list:
                    ...
                    pred = (
                        target_entity_weight * target_entity_log_prob
                        + time_weight * time_log_prob
                    )
                    pred_ground = pred[target_ent]
                    ob_pred_comp1 = (pred > pred_ground).data.cpu().numpy()
                    ob_pred_comp2 = (pred == pred_ground).data.cpu().numpy()
                    target_rank = (
                        np.sum(ob_pred_comp1) + ((np.sum(ob_pred_comp2) - 1.0) / 2) + 1
                    )
...
def eval_time_prediction(
    model, batch_G, static_entity_emb, dynamic_entity_emb, dynamic_relation_emb, args
):
    model.eval()

    with torch.no_grad():
        batch_G_temporal_static_emb = static_entity_emb.temporal[
            batch_G.ndata[dgl.NID].long()
        ].to(args.device)
        batch_G_temporal_dynamic_emb = dynamic_entity_emb.temporal[
            batch_G.ndata[dgl.NID].long()
        ][:, -1, :, :].to(args.device)
        temporal_dynamic_relation_emb = dynamic_relation_emb.temporal[:, -1, :, :].to(
            args.device
        )  # [:, -1, :, :] to retrieve last hidden from rnn

        expected_event_time = model.inter_event_time_model.expected_event_time(
            batch_G,
            batch_G_temporal_dynamic_emb,
            batch_G_temporal_static_emb,
            temporal_dynamic_relation_emb,
        ...
        if full_link_pred_eval:
            ...
            for i, (prior_G, batch_G, cumul_G, batch_times) in enumerate(batch_tqdm):
                ...
                if phase in ["Test", "Validation"]:
                    ...
                    batch_eval_ranks_dict = eval_static_link_prediction(
                        model, cumul_G, static_entity_emb, batch_times, args
                    )
            ...
            # Log the results
            logger.info(
                f"{phase} Metrics: MRR={eval_dict['MRR']:.6f}, Rec@1={eval_dict['REC1']:.6f}, Rec@3={eval_dict['REC3']:.6f}, Rec@10={eval_dict['REC10']:.6f}, Rec@100={eval_dict['REC100']:.6f}"
            )
            ...
            if phase == "Test":
                ...
                with open(
                    os.path.join(
                        log_root_path,
                        f"{args.result_file_prefix}{args.graph}_eval_{args.eval}_static_link_pred_test_result.txt",
                    ),
                    "w",
                ) as f:
                    f.write(
...
def eval_static_link_prediction(model, entire_G, static_entity_emb, eval_times, args):
    ...
    with torch.no_grad():
        ...
        eval_eid = (
            torch.nonzero(
                (eval_time_from <= entire_G.edata["time"])
                & (entire_G.edata["time"] <= eval_time_to),
                as_tuple=False,
            )
            .squeeze()
            .view(-1)
        ...
        with entire_G.local_scope():
            ...
            entire_G_static_emb = static_entity_emb[entire_G.ndata[dgl.NID].long()].to(
                args.device
            )
            # Encoded embeddings for node
            ...
            edges_target_entity_log_prob = (
                torch.log(torch.softmax(-edge_LL, dim=1)).cpu().detach()
            )
            ...
            eval_triples = torch.cat(
                (
                    entire_G.ndata[dgl.NID][entire_G_edges_src[eval_eid].long()].view(
                        -1, 1
                    ),
                    entire_G.edata["rel_type"][eval_eid].view(-1, 1),
                    entire_G.ndata[dgl.NID][entire_G_edges_dst[eval_eid].long()].view(
                        -1, 1
                    ),
                ),
                dim=1,
            ...
            eval_ranks = []
            for (s, r, o), target_entity_log_prob in zip(
                eval_triples, edges_target_entity_log_prob
            ):
                target_ent = o
...
def eval_edges_iet(
    ...
    with torch.no_grad():
        ...
        eval_eid = (
            torch.nonzero(
                (time_from <= cumul_G.edata["time"])
                & (cumul_G.edata["time"] <= time_to),
                as_tuple=False,
            )
            .squeeze()
            .view(-1)
        ...
        eval_edges = torch.cat(
            (eval_src.view(-1, 1), eval_rel.view(-1, 1), eval_dst.view(-1, 1)), dim=1
        )

        eval_dict = evaluator.forward(
            eval_edges,
            eval_edge_time,
            cumul_G.nodes(),
            dynamic_entity_emb,
            static_entity_emb,
            dynamic_relation_emb,
            cumul_G.ndata[dgl.NID],
            entire_G.number_of_nodes(),
            node_latest_event_time,
        )
        assert len(eval_dict["edge_ranks"]) == len(eval_eid), (
            len(eval_dict["edge_ranks"]),
            len(eval_eid),
        )
...
class EdgeEvaluator(nn.Module):
    ...
    def forward(
        ...
        if eval_G_nodes.device == torch.device("cpu"):
            MAX_EDGES = (
                num_nodes * 10
            )  # adjust appropriately according to the CPU memory size
        else:
        ...
        assert len(eval_edges) == len(eval_edge_time), (
            len(eval_edges),
            len(eval_edge_time),
        )
        src, rel, dst = (
            eval_edges[:, 0].type(settings.DGL_GRAPH_ID_TYPE),
            eval_edges[:, 1],
            eval_edges[:, 2].type(settings.DGL_GRAPH_ID_TYPE),
        )
        ...
        for i, (
            ...
            eval_rel_chunk,
            eval_dst_chunk,
            eval_edge_time_chunk,
        ) in enumerate(zip(src_chunks, rel_chunks, dst_chunks, edge_time_chunks)):
            if perturb_dst:
            ...
            assert len(eval_src_chunk) == len(eval_dst_chunk), (
                len(eval_src_chunk),
                len(eval_dst_chunk),
            )
            edge_scores = edge_scores.view(
                len(eval_src_chunk), -1
            )  # [# src/dst entities in a chunk, # nodes in a graph]

            edge_scores_all.append(edge_scores.cpu().detach())

            if perturb_dst:
                true_edge_scores = edge_scores.gather(
                    1, eval_dst_chunk.view(-1, 1).long()
                )
            else:
                true_edge_scores = edge_scores.gather(
                    1, eval_src_chunk.view(-1, 1).long()
                )
            # noinspection PyUnresolvedReferences
            true_edge_ranks = ((edge_scores > true_edge_scores).sum(dim=1) + 1) + (
                (edge_scores == true_edge_scores).sum(dim=1) - 1.0
            ) / 2
            true_edge_ranks = true_edge_ranks.cpu().detach().tolist()
            assert len(true_edge_ranks) == len(eval_src_chunk) == len(eval_dst_chunk), (
                len(true_edge_ranks),
                len(eval_src_chunk),
                len(eval_dst_chunk),
            )
        ...
        eval_dict = {
            "edge_scores": torch.cat(
                edge_scores_all, dim=0
            ),  # tensor of shape (# edges, # nodes)
            "edge_ranks": edge_ranks_all,  # list of length=# edges
        }
        assert eval_dict["edge_scores"].shape == (len(eval_edges), num_nodes), (
            eval_dict["edge_scores"].shape,
            (len(eval_edges), num_nodes),
        )
        assert len(eval_dict["edge_ranks"]) == len(eval_edges), (
            len(eval_dict["edge_ranks"]),
            len(eval_edges),
        )
...
def compute_loss(
    ...
    if batch_eid is not None:
        ...
        sub_batch_G = dgl.edge_subgraph(
            batch_G, batch_eid.type(settings.DGL_GRAPH_ID_TYPE), preserve_nodes=False
        )
        sub_batch_G.ndata[dgl.NID] = batch_G.ndata[dgl.NID][
            sub_batch_G.ndata[dgl.NID].long()
        ]  # map nid in sub_batch_G to nid in the full graph
    ...
    if loss in ["edge", "both"]:
        ...
        sub_batch_G_structural_dynamic_entity_emb = dynamic_entity_emb.structural[
            sub_batch_G.ndata[dgl.NID].long()
        ][:, -1, :].to(args.device)  # [:, -1, :] to retrieve last hidden from rnn
        sub_batch_G_combined_emb = model.combiner(
            sub_batch_G_structural_static_entity_emb,
        ...
        edge_LL = model.edge_model(
            sub_batch_G,
            sub_batch_G_combined_emb,
            eid=batch_eid,
    ...
    if loss in ["time", "both"]:
        sub_batch_G_temporal_static_entity_emb = static_entity_emb.temporal[
            sub_batch_G.ndata[dgl.NID].long()
        ].to(args.device)
        sub_batch_G_temporal_dynamic_entity_emb = dynamic_entity_emb.temporal[
            sub_batch_G.ndata[dgl.NID].long()
        ][:, -1, :, :].to(args.device)  # [:, -1, :, :] to retrieve last hidden from rnn
        temporal_dynamic_relation_emb = dynamic_relation_emb.temporal[:, -1, :, :].to(
            args.device
