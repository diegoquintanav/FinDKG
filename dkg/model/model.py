...
from dkg.model.embedding import EmbeddingUpdater
...
from dkg.utils.train_utils import activation_string
...
class DynamicKGEngine:
    def __init__(self, args, num_nodes, model_type="KGT+RNN"):
        ...
        self.node_latest_event_time = torch.zeros(
            num_nodes, num_nodes + 1, 2, dtype=args.inter_event_dtype
        )
    ...
    def init_embeddings(self):
        ...
        static_entity_embeds = MultiAspectEmbedding(
            structural=get_embedding(
                self.num_nodes, args.static_entity_embed_dim, zero_init=False
            ),
    ...
    def build_dynamic_graph_model(self):
        ...
        self.inter_event_time_model = InterEventTimeModel(
            dynamic_entity_embed_dim=self.args.temporal_dynamic_entity_embed_dim,
        ...
        return DynamicGraphModel(
            self.embedding_updater,
            self.combiner,
            self.edge_model,
...
def predict_link(
    ...
    heads_local = [
        np.argwhere(cumul_G.ndata[dgl.NID].numpy() == x)[0][0] for x in tgt_heads
    ]
    tails_local = [
        np.argwhere(cumul_G.ndata[dgl.NID].numpy() == y)[0][0] for y in tgt_tails
    ]
...
class Combiner(nn.Module):
    def __init__(
        ...
        self.static_dynamic_combiner = StaticDynamicCombiner(
            static_dynamic_combine_mode, static_emb_dim, dynamic_emb_dim
        )
    ...
    def forward(self, static_emb, dynamic_emb, G=None):
        ...
        combined_emb = self.static_dynamic_combiner(
            self.dropout(static_emb), dynamic_emb
        )
        return combined_emb
...
class StaticDynamicCombiner(nn.Module):
    ...
    def __repr__(self):
        return (
            "%s(mode=%s, static_emb_dim=%d, dynamic_emb_dim=%d, combined_emb_dim=%d)"
            % (
                self.__class__.__name__,
                self.mode,
                self.static_emb_dim,
                self.dynamic_emb_dim,
                self.combined_emb_dim,
            )
        )
...
class GraphReadout(nn.Module):
    def __init__(
        self, combiner: Combiner, readout_op="max", readout_node_type="static"
    ):
        super().__init__()
        ...
        if readout_op in ["max", "min", "mean"]:
            ...
            self.node_gating = nn.Sequential(
                nn.Linear(self.node_emb_dim, 1), nn.Sigmoid()
            )
    ...
    def forward(self, G, combined, static, dynamic):
        with G.local_scope():
            ...
            if self.readout_op in ["max", "min", "mean"]:
                ...
                return (self.node_gating(node_emb) * self.node_to_graph(node_emb)).sum(
                    0, keepdim=True
                )
            else:
...
class EdgeModel(nn.Module):
    def __init__(
        ...
        node_graph_rel_emb_dim = (
            self.combined_emb_dim + self.graph_readout.graph_emb_dim + rel_embed_dim * 2
        )
    ...
    def log_prob_head(self, graph_emb, G, edge_head):
        ...
        return -self.criterion(
            head_pred, G.ndata[dgl.NID][edge_head.long()].long()
        ), head_pred
    ...
    def log_prob_tail(
        ...
        return -self.criterion(
            tail_pred, G.ndata[dgl.NID][edge_tail.long()].long()
        ), tail_pred

    def graph_emb(self, G, combined_emb, static_emb, dynamic_emb):
        return self.graph_readout.forward(
            G,
            ("emb", combined_emb),
    ...
    def forward(
        ...
        with G.local_scope():
            ...
            dynamic_rel_emb = dynamic_relation_emb[
                :, :, 1
            ]  # use (relation-dest) context
            graph_emb = self.graph_emb(G, combined_emb, static_emb, dynamic_emb)
            log_prob_tail, tail_pred = self.log_prob_tail(
                edge_head_emb, graph_emb, edge_rel, G, edge_tail, dynamic_rel_emb
            )
            log_prob_rel, rel_pred = self.log_prob_rel(
                edge_head_emb, graph_emb, edge_rel
            )
...
class StaticEdgeModel(nn.Module):
    def __init__(self, num_entities, num_rels, rel_embed_dim, combiner, dropout=0.0):
        ...
        self.graph_readout = GraphReadout(
            combiner, readout_op="max", readout_node_type="static"
        )
        graph_emb_dim = self.graph_readout.graph_emb_dim
        ...
        self.transform_tail = nn.Sequential(
            nn.Linear(
                graph_emb_dim + rel_embed_dim, 2 * (graph_emb_dim + rel_embed_dim)
            ),
            nn.Tanh(),
    ...
    def forward(self, G, static_emb, eid=None, return_pred=False):
        with G.local_scope():
            ...
            graph_emb = self.graph_readout(
                G, None, static_emb, None
            )  # Assume that this works similarly to Dynamic Edge Model

            log_prob_tail, tail_pred = self.log_prob_tail(
                graph_emb, edge_rel, G, edge_tail
            )
...
class InterEventTimeModel(nn.Module):
    def __init__(
        ...
        self.tpp_model = LogNormMixTPP(
            ...
            num_mix_components,
            time_interval_transform,
            mean_log_inter_event_time,
            std_log_inter_event_time,
            dropout,
        )
    ...
    def log_prob_density(
        ...
        return self.tpp_model.log_prob_density(
            batch_G,
            dynamic_entity_emb,
            static_entity_emb,
            dynamic_relation_emb,
    ...
    def log_prob_interval(
        ...
        return self.tpp_model.log_prob_interval(
            batch_G,
            dynamic_entity_emb,
            static_entity_emb,
            dynamic_relation_emb,
    ...
    def expected_event_time(
        ...
        return self.tpp_model.expected_event_time(
            batch_G,
            dynamic_entity_emb,
            static_entity_emb,
            dynamic_relation_emb,
