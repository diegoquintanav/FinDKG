...
from dkg.utils.model_utils import node_norm_to_edge_norm

MultiAspectEmbedding = namedtuple(
    "MultiAspectEmbedding", ["structural", "temporal"], defaults=[None, None]
)
...
class EmbeddingUpdater(nn.Module):
    ...
    def __init__(
        ...
        num_rels,
        rel_embed_dim,
        graph_structural_conv="RGCN+RNN",
        graph_temporal_conv="RGCN+RNN",
        num_gconv_layers=2,
        num_rnn_layers=1,
        num_node_types=1,
        num_heads=8,
        ...
        self.structural_relation_rnn = RelationRNN(
            "RNN", num_rnn_layers, in_dim, rel_embed_dim, num_rels, dropout
        )
        self.temporal_relation_rnn = RelationRNN(
            "RNN", num_rnn_layers, in_dim, rel_embed_dim, num_rels, dropout
        )
    ...
    def forward(
        ...
        if batch_node_indices is None:
            batch_node_indices = (
                batch_G.nodes().long()
            )  # update embeddings of all nodes in batch_G
        ...
        batch_structural_dynamic_relation_emb = self.structural_relation_rnn.forward(
            batch_G,
            dynamic_relation_emb.structural,
            static_entity_emb.structural,
            device,
        )
        batch_temporal_dynamic_relation_emb = self.temporal_relation_rnn.forward(
            batch_G, dynamic_relation_emb.temporal, static_entity_emb.temporal, device
        )
        ...
        if batch_structural_dynamic_entity_emb is not None:
            ...
            updated_structural[batch_G.ndata[dgl.NID][batch_node_indices].long()] = (
                batch_structural_dynamic_entity_emb.cpu()
            )
        ...
        if batch_temporal_dynamic_entity_emb is not None:
            ...
            updated_temporal[batch_G.ndata[dgl.NID][batch_node_indices].long()] = (
                batch_temporal_dynamic_entity_emb.cpu()
            )
        updated_dynamic_entity_emb = MultiAspectEmbedding(
            structural=updated_structural, temporal=updated_temporal
        )
        ...
        if batch_structural_dynamic_relation_emb is not None:
            ...
            updated_structural[batch_G_uniq_rel] = (
                batch_structural_dynamic_relation_emb.cpu()
            )
        ...
        if batch_temporal_dynamic_relation_emb is not None:
            ...
            updated_temporal[batch_G_uniq_rel] = (
                batch_temporal_dynamic_relation_emb.cpu()
            )
...
class GraphStructuralRNNConv(nn.Module):
    ...
    def forward(
        ...
        batch_structural_static_entity_emb = static_entity_emb.structural[
        ...
        # KGTransformer
            ...
            conv_structural_static_emb = self.graph_conv(
                batch_G, batch_structural_static_entity_emb
            )  # shape=(# nodes in batch_G, dim-hidden)
        ...
        if self.add_entity_emb:
            structural_rnn_input.append(
                static_entity_emb.structural[
                    batch_G.ndata[dgl.NID][batch_node_indices].long()
                ].to(device)
            )
        structural_rnn_input = torch.cat(structural_rnn_input, dim=1).unsqueeze(1)

        # Update structural dynamics
        structural_dynamic = dynamic_entity_emb.structural[
            batch_G.ndata[dgl.NID][batch_node_indices].long()
        ]
        structural_dynamic = structural_dynamic.to(device)

        output, hn = self.rnn_structural(
            structural_rnn_input, structural_dynamic.transpose(0, 1).contiguous()
        )  # transpose to make shape to be (num_layers, batch, hidden_size)
        updated_structural_dynamic_entity_emb = hn.transpose(
            0, 1
        )  # transpose to make shape to be (batch, num_layers, hidden_size)
...
class GraphTemporalRNNConv(nn.Module):
    ...
    def forward(
        ...
        EventTimeHelper.get_inter_event_times(
            batch_G, self.node_latest_event_time[..., 0], update_latest_event_time=True
        )
        ...
        rev_batch_G_sparse_inter_event_times = (
            EventTimeHelper.get_sparse_inter_event_times(
                rev_batch_G, self.node_latest_event_time[..., 1]
            )
        )
        EventTimeHelper.get_inter_event_times(
            rev_batch_G,
            self.node_latest_event_time[..., 1],
            update_latest_event_time=True,
        )

        """Temporal RNN input"""
        batch_temporal_static_entity_emb = static_entity_emb.temporal[
            batch_G.ndata[dgl.NID].long()
        ].to(device)
        edge_norm = (
            1
            / self.time_interval_transform(batch_G_sparse_inter_event_times).clamp(
                min=1e-10
            )
        ).clamp(max=10.0)
        ...
        if isinstance(self.graph_conv, RGCN):
            batch_G_conv_temporal_static_emb = self.graph_conv(
                batch_G,
            ...
            raise ValueError(" <-- Invalid graph conv")
        ...
        rev_batch_temporal_static_entity_emb = static_entity_emb.temporal[
            rev_batch_G.ndata[dgl.NID].long()
        ].to(device)
        rev_edge_norm = (
            1
            / self.time_interval_transform(rev_batch_G_sparse_inter_event_times).clamp(
                min=1e-10
            )
        ).clamp(max=10.0)
        ...
        if isinstance(self.graph_conv, RGCN):
            rev_batch_G_conv_temporal_static_emb = self.graph_conv(
                rev_batch_G,
                rev_batch_temporal_static_entity_emb,
                batch_G.edata["rel_type"].long(),
                rev_edge_norm,
            )
        elif isinstance(self.graph_conv, KGTransformer):
            rev_batch_G_conv_temporal_static_emb = self.graph_conv(
                ...
                rev_batch_G.edata["rel_type"].long(),
            )  # num of relationship
        else:
            raise ValueError(" <-- Invalid graph conv")
        ...
        temporal_dynamic = dynamic_entity_emb.temporal[
            batch_G.ndata[dgl.NID][batch_node_indices].long()
        ].to(device)
        ...
        output, hn = self.rnn_temporal(
            temporal_rnn_input_batch_G,
            temporal_dynamic_batch_G.transpose(0, 1).contiguous(),
        )  # transpose to make shape to be (num_layers, batch, hidden_size)
        updated_temporal_dynamic_batch_G = hn.transpose(
            0, 1
        )  # transpose to make shape to be (batch, num_layers, hidden_size)

        output, hn = self.rnn_temporal(
            temporal_rnn_input_rev_batch_G,
            temporal_dynamic_rev_batch_G.transpose(0, 1).contiguous(),
        )  # transpose to make shape to be (num_layers, batch, hidden_size)
        updated_temporal_dynamic_rev_batch_G = hn.transpose(
            0, 1
        )  # transpose to make shape to be (batch, num_layers, hidden_size)

        updated_temporal_dynamic_entity_emb = torch.cat(
            [
                updated_temporal_dynamic_batch_G.unsqueeze(-1),
                updated_temporal_dynamic_rev_batch_G.unsqueeze(-1),
            ],
            dim=-1,
        )
...
class RelationRNN(nn.Module):
    def __init__(
        self, rnn, num_rnn_layers, rnn_in_dim, rnn_hid_dim, num_rels, dropout=0.0
    ):
        super().__init__()
        if num_rnn_layers == 1:
            dropout = 0.0
    ...
    def forward(self, batch_G, dynamic_relation_emb, static_entity_emb, device):
        ...
        batch_G_src_emb_avg_by_rel_ = scatter_mean(
            static_entity_emb[batch_G_src_nid].to(device).transpose(0, 1), batch_G_rel
        ).transpose(0, 1)  # shape=(max rel in batch_G, static entity emb dim)
        batch_G_dst_emb_avg_by_rel_ = scatter_mean(
            static_entity_emb[batch_G_dst_nid].to(device).transpose(0, 1), batch_G_rel
        ).transpose(0, 1)  # shape=(max rel in batch_G, static entity emb dim)

        # filter out relations that are non-existent in batch_G
        batch_G_uniq_rel = torch.unique(batch_G_rel, sorted=True)
        batch_G_src_emb_avg_by_rel = batch_G_src_emb_avg_by_rel_[
            batch_G_uniq_rel
        ]  # shape=(# uniq rels in batch_G, static entity emb dim)
        batch_G_dst_emb_avg_by_rel = batch_G_dst_emb_avg_by_rel_[
            batch_G_uniq_rel
        ]  # shape=(# uniq rels in batch_G, static entity emb dim)

        batch_G_dynamic_relation_emb = dynamic_relation_emb[batch_G_uniq_rel]
        batch_G_src_dynamic_relation_emb = batch_G_dynamic_relation_emb[..., 0].to(
            device
        )
        batch_G_dst_dynamic_relation_emb = batch_G_dynamic_relation_emb[..., 1].to(
            device
        )

        output, hn = self.rnn_relation(
            batch_G_src_emb_avg_by_rel.unsqueeze(1),
            batch_G_src_dynamic_relation_emb.transpose(0, 1).contiguous(),
        )  # transpose to make shape to be (num_layers, batch, hidden_size)
        updated_batch_G_src_dynamic_relation_emb = hn.transpose(
            0, 1
        )  # transpose to make shape to be (batch, num_layers, hidden_size)
        output, hn = self.rnn_relation(
            batch_G_dst_emb_avg_by_rel.unsqueeze(1),
            batch_G_dst_dynamic_relation_emb.transpose(0, 1).contiguous(),
        )  # transpose to make shape to be (num_layers, batch, hidden_size)
        updated_batch_G_dst_dynamic_relation_emb = hn.transpose(
            0, 1
        )  # transpose to make shape to be (batch, num_layers, hidden_size)
        updated_batch_G_dynamic_relation_emb = torch.cat(
            [
                updated_batch_G_src_dynamic_relation_emb.unsqueeze(-1),
                updated_batch_G_dst_dynamic_relation_emb.unsqueeze(-1),
            ],
            dim=-1,
        )
...
class EventTimeHelper:
    @classmethod
    def get_sparse_inter_event_times(
        cls, batch_G, node_latest_event_time, _global=False
    ):
        batch_sparse_latest_event_times = cls.get_sparse_latest_event_times(
            batch_G, node_latest_event_time, _global
        )
        return batch_G.edata["time"] - batch_sparse_latest_event_times

    @classmethod
    def get_sparse_latest_event_times(
        cls, batch_G, node_latest_event_time, _global=False
    ):
        ...
        if _global:
            ...
            return batch_latest_event_time[
                batch_G_dst.long(), batch_G_nid[batch_G_src.long()]
            ].to(device)

    @classmethod
    def get_inter_event_times(
        cls, batch_G, node_latest_event_time, update_latest_event_time=True
    ):
        batch_G_nid = batch_G.ndata[dgl.NID].long()
        batch_latest_event_time = node_latest_event_time[batch_G_nid]
        ...
        batch_inter_event_times = torch.zeros(
            batch_G.num_nodes(),
            batch_G.num_all_nodes + 1,
            dtype=settings.INTER_EVENT_TIME_DTYPE,
        ).to(device)
        batch_inter_event_times[batch_G_dst, batch_G_nid[batch_G_src]] = (
            batch_G_time
            - batch_latest_event_time[batch_G_dst, batch_G_nid[batch_G_src]].to(device)
        )

        batch_G.update_all(fn.copy_e("time", "t"), fn.max("t", "max_event_time"))
        batch_G_max_event_time = batch_G.ndata["max_event_time"].to(
            settings.INTER_EVENT_TIME_DTYPE
        )

        batch_max_latest_event_time = batch_latest_event_time[:, -1].to(device)
        batch_G_max_event_time = torch.max(
            batch_G_max_event_time, batch_max_latest_event_time
        )
        batch_inter_event_times[:, -1] = (
            batch_G_max_event_time - batch_max_latest_event_time
        )

        if update_latest_event_time:
            node_latest_event_time[
                batch_G_nid[batch_G_dst], batch_G_nid[batch_G_src]
            ] = batch_G_time.cpu()
...
class StaticEmbeddingUpdater(nn.Module):
    def __init__(
        ...
        self.graph_structural_conv = RGCN(
            in_dim,
            self.structural_hid_dim,
            self.structural_hid_dim,
            n_layers=num_gconv_layers,
            num_rels=self.num_rels,
            regularizer="bdd",
