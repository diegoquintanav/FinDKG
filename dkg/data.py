...
def load_temporal_knowledge_graph(
    graph_name, idtype=settings.DGL_GRAPH_ID_TYPE, data_root=settings.DATA_ROOT
) -> dgl.DGLGraph:
    ...
    train_data_table = load_data_table(
        graph_name, train_file, column_names, data_root=data_root
    )
    val_data_table = load_data_table(
        graph_name, val_file, column_names, data_root=data_root
    )
    test_data_table = load_data_table(
        graph_name, test_file, column_names, data_root=data_root
    )
    all_data_table = pd.concat(
        [train_data_table, val_data_table, test_data_table], ignore_index=True
    )

    stat_table = load_data_table(
        graph_name,
        "stat.txt",
    ...
    num_entities, num_relations = (
        stat_table["num_entities"].item(),
        stat_table["num_relations"].item(),
    )
    ...
    if graph_name in ["FinDKG", "FinDKG-live"]:
        ...
        node_data_table = load_data_table(
            graph_name,
            "entity2id.txt",
            column_names=["entity", "entity_id", "ntype", "ntype_id"],
            data_root=data_root,
        )
        G.ndata["node_type"] = torch.from_numpy(
            node_data_table["ntype_id"].to_numpy()
        ).type(idtype)
    else:
        G.num_node_types = 1
    ...
    val_edge_mask = get_edge_mask(
        num_edges, len(train_data_table), len(train_data_table) + len(val_data_table)
    )
    test_edge_mask = get_edge_mask(
        num_edges, len(train_data_table) + len(val_data_table), num_edges
    )
...
def load_data_table(
    graph_name, file_name, column_names=None, data_root=settings.DATA_ROOT
):
